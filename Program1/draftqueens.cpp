...
int main()
{	  
    board[0][0] = 1; //place the first queen on the board (= 1 means occupied)
	...
	int row = 0;
	int i = 4;
	for (int col = 0; (col + i) < 8; i--) 
	{
		col += i;
		row++;
		bool canoccupy = checkSafety(row, col);
