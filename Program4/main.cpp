  ...
  int segmentCounts[1000], int *segmentPos[1000], Message *messages)
{
  ...
  while(inf >> messages[messageCount].time >> messages[messageCount].portNum)
  {
    ...
    ptr = strtok(line, " \n");
     ...
     while(ptr)
     {
      ...
      {
        memcpy(&stream[i * PACKET_SIZE], &data[segmentPos[messages[messageCount].portNum][packetNums[i]]], PACKET_SIZE);
 ...
 int main(int argc, char **argv)
 {
   ...
   messageCount = readFile("C:\\Users\\matta_000\\Documents\\CodeBlocks\\ecs60_4_3\\tcpFile1.txt", &numPorts, portNums, data, segmentCounts, segmentPos,  messages);
       ...
       if(streamSize != messages[i].packetCount * PACKET_SIZE)
         ...
         if(messages[i].packetCount > 0
           ...
           cout << "Error in stream contents for message #"  << i
            ...
            // cout << stream[messages[i].messageCharPos] << ": " << messages[i].messageChar << endl;
     } // else a test message
   }
